...
def get_implied_policy(policy,num_actions): #This only works for num_actions=2 or 1 currently
    act_sp = policy.size
    all_actions = get_combinatorial_actions(act_sp,num_actions)
    impl_policy = np.zeros(len(all_actions))
    for i in range(len(all_actions)):
        ...
        pminus = policy.copy()
        pminus[a_1] = 0
        pminus = act_sp/(act_sp-1)*pminus
        p_12 = p_1*pminus[a_2]
        pminus = policy.copy()
        pminus[a_2] = 0
        pminus = act_sp/(act_sp-1)*pminus
        p_21 = p_2*pminus[a_1]
        impl_policy[i] = p_12 + p_21
    return impl_policy