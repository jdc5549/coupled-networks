...
switch opt.optimizer
    ...
    case 'mexosi'
        ...
        b_min = [b_pf; -Inf(size(b_flow_1)); -Inf(size(b_flow_2))];
...
if any( round(Pg)<round(Pg_min-EPS) | round(Pg)>round(Pg_max+EPS) )    
    trouble_low = find(Pg+EPS < Pg_min);
    trouble_high = find(Pg > Pg_max+EPS);
    to_screen = ['outside bounds low index: ',num2str(trouble_low),', outside bounds high index: ',...
        num2str(trouble_high),', outside bounds high Pg: ',num2str(Pg(trouble_high)),', outside bounds high Pg0: ',num2str(Pg0(trouble_high))];
    display(to_screen);
...
if any( Pd0>0 & ( round(Pd) < round(0-EPS) | round(Pd) > round(Pd0+EPS) ) )
    %TODO count comm_status and report with error.
    trouble_low = find(Pd+EPS < 0);
    trouble_high = find(Pd > Pd0+EPS);
    sizes = ['size Pd ',num2str(size(Pd)),', size Pg ',num2str(size(Pg))];
    display(sizes);
    to_screen_low = ['Pd outside bounds low index: ',num2str(trouble_low.'),', outside bounds low Pd: ',num2str(Pd(trouble_low).'),...
        ', Pd0 low: ',num2str(Pd0(trouble_low).')];
    to_screen_high = [', Pd outside bounds high index: ',num2str(trouble_high.'),...
        ', outside bounds high Pd: ',num2str(Pd(trouble_high).'),', Pd0 high: ',num2str(Pd0(trouble_high).')];
    to_screen = [to_screen_low, to_screen_high];
    display(to_screen);
