% function cn_runner_q(exp_id, config_json)
function [is_blackout,GC_size,MW_lost] = cn_runner_q(exp_id, config_json, exps_to_run_file, starting_run)
% Inputs:
...
%% Get the outage point
p_values = opt.comm.pValues;
if p_values ~= 1
    error('Only use cn_runner_q for sweeps across q, not p. Exiting!');
    exit;
end
p_min = opt.comm.pMin;
p_point = p_min;
...
q_values = opt.comm.qValues;
q_min = opt.comm.qMin;
q_range = opt.comm.qMax - q_min;
results = zeros(q_values,4);

for i = 0:q_values
    tic;
    q_point = i/q_values;
    q_point = q_point/(1/q_range)+q_min;
    
    %% Get the coupled nodes and write them to file
    coupled_nodes = get_coupled_nodes_from_file(exp_id,q_point,opt,n);
    ...
    sprintf('Single coupled nodes file name at q_point %f with %d coupled nodes: %s', q_point, size(coupled_nodes,1), coupled_nodes_filename)
    ...
    if opt.sim.use_comm_model || opt.sim.use_control
    ...
    results(i+1,:) = [q_point,is_blackout,GC_size,MW_lost];
    ...
    fprintf('Run time for coupling point %.3f in experiment %s was %.2fs\n',q_point,exp_id,toc);
...
if p_column > numOutagesInCouplingFile || p_column == 0 || n ~= 2383
    ...
    try
        ...
        if opt.verbose == true
            fprintf('Size coupled_nodes list %d for run %s, q_point %f, q_column %d\n',num_coupled_nodes,exp_id,1-q_point,p_column);
        end
    catch
