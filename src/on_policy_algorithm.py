...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if self.env != None and self.env.get_attr('name')[0] == "CoupledNetsEnv-v0":
            self.rollout_buffer = buffer_cls(
                self.n_steps*int(self.env.get_attr('num_nodes_attacked')[0]/self.env.get_attr('attack_degree')[0]),
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            #clipped_actions = clipped_actions.flatten()
            ...
            if env.get_attr('name')[0] == "CoupledNetsEnv-v0":
                for j in range(num_samples):
                    a = np.asarray([actions[i][j] for i in range(actions.shape[0])])
                    v = th.tensor([values[i][j] for i in range(values.shape[0])])
                    lp = th.tensor([log_probs[i][j] for i in range(log_probs.shape[0])])
                    if len(a.shape)<2:
                        a = np.expand_dims(a,1)
                        v = th.unsqueeze(v,1)
                    rollout_buffer.add(self._last_obs, a, rewards, self._last_episode_starts, v, lp)
            else:
